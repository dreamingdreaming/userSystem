...
(function()
{
	...
	var plugin = ( CKEDITOR.plugins.pastefromword =
	{
		...
		filters :
		{
				...
				/**
				 * A simple li.ren.filter which always rejecting.
				 */
				...
				/**
				 * A li.ren.filter dedicated on the 'style' attribute filtering, e.g. dropping/replacing style properties.
				...
				/**
				 * A li.ren.filter which remove cke-namespaced-attribute on
				...
				/**
				 * A li.ren.filter which will be used to apply inline css style according the stylesheet
	...
	CKEDITOR.cleanWord = function( data, editor )
	{
		...
		// Allow extending data li.ren.filter rules.
		...
		/* Below post processing those things that are unable to delivered by li.ren.filter rules. */

		// Remove 'cke' namespaced attribute used in li.ren.filter rules as marker.
